...
// ClusterVmAntiAffinityRuleReconciler reconciles a ClusterVmAntiAffinityRule object
type ClusterVmAntiAffinityRuleReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=datastore.vsphere.kubeform.com,resources=clustervmantiaffinityrules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=datastore.vsphere.kubeform.com,resources=clustervmantiaffinityrules/status,verbs=get;update;patch

func (r *ClusterVmAntiAffinityRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("clustervmantiaffinityrule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ClusterVmAntiAffinityRule")
...
func (r *ClusterVmAntiAffinityRuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &datastorev1alpha1.ClusterVmAntiAffinityRule{}); err != nil {
			klog.Error(err, "unable to set up auditor", datastorev1alpha1.ClusterVmAntiAffinityRule{}.APIVersion, datastorev1alpha1.ClusterVmAntiAffinityRule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&datastorev1alpha1.ClusterVmAntiAffinityRule{}).
