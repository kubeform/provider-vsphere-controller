...
// MachineSnapshotReconciler reconciles a MachineSnapshot object
...
// +kubebuilder:rbac:groups=virtual.vsphere.kubeform.com,resources=machinesnapshots,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=virtual.vsphere.kubeform.com,resources=machinesnapshots/status,verbs=get;update;patch

func (r *MachineSnapshotReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("machinesnapshot", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch MachineSnapshot")
...
func (r *MachineSnapshotReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &virtualv1alpha1.MachineSnapshot{}); err != nil {
			klog.Error(err, "unable to set up auditor", virtualv1alpha1.MachineSnapshot{}.APIVersion, virtualv1alpha1.MachineSnapshot{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&virtualv1alpha1.MachineSnapshot{}).
