...
// +kubebuilder:rbac:groups=vmfs.vsphere.kubeform.com,resources=datastores,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=vmfs.vsphere.kubeform.com,resources=datastores/status,verbs=get;update;patch

func (r *DatastoreReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("datastore", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Datastore")
...
func (r *DatastoreReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &vmfsv1alpha1.Datastore{}); err != nil {
			klog.Error(err, "unable to set up auditor", vmfsv1alpha1.Datastore{}.APIVersion, vmfsv1alpha1.Datastore{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&vmfsv1alpha1.Datastore{}).
