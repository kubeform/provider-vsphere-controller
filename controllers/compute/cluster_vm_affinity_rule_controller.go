...
// ClusterVmAffinityRuleReconciler reconciles a ClusterVmAffinityRule object
type ClusterVmAffinityRuleReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=compute.vsphere.kubeform.com,resources=clustervmaffinityrules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=compute.vsphere.kubeform.com,resources=clustervmaffinityrules/status,verbs=get;update;patch

func (r *ClusterVmAffinityRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("clustervmaffinityrule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ClusterVmAffinityRule")
...
func (r *ClusterVmAffinityRuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &computev1alpha1.ClusterVmAffinityRule{}); err != nil {
			klog.Error(err, "unable to set up auditor", computev1alpha1.ClusterVmAffinityRule{}.APIVersion, computev1alpha1.ClusterVmAffinityRule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&computev1alpha1.ClusterVmAffinityRule{}).
