...
// ClusterVmDependencyRuleReconciler reconciles a ClusterVmDependencyRule object
type ClusterVmDependencyRuleReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=compute.vsphere.kubeform.com,resources=clustervmdependencyrules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=compute.vsphere.kubeform.com,resources=clustervmdependencyrules/status,verbs=get;update;patch

func (r *ClusterVmDependencyRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("clustervmdependencyrule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ClusterVmDependencyRule")
...
func (r *ClusterVmDependencyRuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &computev1alpha1.ClusterVmDependencyRule{}); err != nil {
			klog.Error(err, "unable to set up auditor", computev1alpha1.ClusterVmDependencyRule{}.APIVersion, computev1alpha1.ClusterVmDependencyRule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&computev1alpha1.ClusterVmDependencyRule{}).
