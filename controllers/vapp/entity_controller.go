...
// +kubebuilder:rbac:groups=vapp.vsphere.kubeform.com,resources=entities,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=vapp.vsphere.kubeform.com,resources=entities/status,verbs=get;update;patch

func (r *EntityReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("entity", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Entity")
...
func (r *EntityReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &vappv1alpha1.Entity{}); err != nil {
			klog.Error(err, "unable to set up auditor", vappv1alpha1.Entity{}.APIVersion, vappv1alpha1.Entity{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&vappv1alpha1.Entity{}).
