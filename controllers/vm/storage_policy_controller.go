...
// +kubebuilder:rbac:groups=vm.vsphere.kubeform.com,resources=storagepolicies,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=vm.vsphere.kubeform.com,resources=storagepolicies/status,verbs=get;update;patch

func (r *StoragePolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("storagepolicy", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch StoragePolicy")
...
func (r *StoragePolicyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &vmv1alpha1.StoragePolicy{}); err != nil {
			klog.Error(err, "unable to set up auditor", vmv1alpha1.StoragePolicy{}.APIVersion, vmv1alpha1.StoragePolicy{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&vmv1alpha1.StoragePolicy{}).
