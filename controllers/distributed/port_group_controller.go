...
// PortGroupReconciler reconciles a PortGroup object
type PortGroupReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=distributed.vsphere.kubeform.com,resources=portgroups,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=distributed.vsphere.kubeform.com,resources=portgroups/status,verbs=get;update;patch

func (r *PortGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("portgroup", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch PortGroup")
...
func (r *PortGroupReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &distributedv1alpha1.PortGroup{}); err != nil {
			klog.Error(err, "unable to set up auditor", distributedv1alpha1.PortGroup{}.APIVersion, distributedv1alpha1.PortGroup{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&distributedv1alpha1.PortGroup{}).
