...
// VirtualSwitchReconciler reconciles a VirtualSwitch object
type VirtualSwitchReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=distributed.vsphere.kubeform.com,resources=virtualswitches,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=distributed.vsphere.kubeform.com,resources=virtualswitches/status,verbs=get;update;patch

func (r *VirtualSwitchReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("virtualswitch", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch VirtualSwitch")
...
func (r *VirtualSwitchReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &distributedv1alpha1.VirtualSwitch{}); err != nil {
			klog.Error(err, "unable to set up auditor", distributedv1alpha1.VirtualSwitch{}.APIVersion, distributedv1alpha1.VirtualSwitch{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&distributedv1alpha1.VirtualSwitch{}).
