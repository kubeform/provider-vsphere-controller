...
// HostOverrideReconciler reconciles a HostOverride object
type HostOverrideReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=dpm.vsphere.kubeform.com,resources=hostoverrides,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dpm.vsphere.kubeform.com,resources=hostoverrides/status,verbs=get;update;patch

func (r *HostOverrideReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("hostoverride", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch HostOverride")
...
func (r *HostOverrideReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dpmv1alpha1.HostOverride{}); err != nil {
			klog.Error(err, "unable to set up auditor", dpmv1alpha1.HostOverride{}.APIVersion, dpmv1alpha1.HostOverride{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dpmv1alpha1.HostOverride{}).
