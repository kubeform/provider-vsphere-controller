...
// +kubebuilder:rbac:groups=custom.vsphere.kubeform.com,resources=attributes,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=custom.vsphere.kubeform.com,resources=attributes/status,verbs=get;update;patch

func (r *AttributeReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("attribute", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Attribute")
...
func (r *AttributeReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &customv1alpha1.Attribute{}); err != nil {
			klog.Error(err, "unable to set up auditor", customv1alpha1.Attribute{}.APIVersion, customv1alpha1.Attribute{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&customv1alpha1.Attribute{}).
