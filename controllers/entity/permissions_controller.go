...
// PermissionsReconciler reconciles a Permissions object
...
// +kubebuilder:rbac:groups=entity.vsphere.kubeform.com,resources=permissions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=entity.vsphere.kubeform.com,resources=permissions/status,verbs=get;update;patch

func (r *PermissionsReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("permissions", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Permissions")
...
func (r *PermissionsReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &entityv1alpha1.Permissions{}); err != nil {
			klog.Error(err, "unable to set up auditor", entityv1alpha1.Permissions{}.APIVersion, entityv1alpha1.Permissions{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&entityv1alpha1.Permissions{}).
