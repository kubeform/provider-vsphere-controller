...
// +kubebuilder:rbac:groups=vnic.vsphere.kubeform.com,resources=vnics,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=vnic.vsphere.kubeform.com,resources=vnics/status,verbs=get;update;patch

func (r *VnicReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("vnic", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Vnic")
...
func (r *VnicReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &vnicv1alpha1.Vnic{}); err != nil {
			klog.Error(err, "unable to set up auditor", vnicv1alpha1.Vnic{}.APIVersion, vnicv1alpha1.Vnic{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&vnicv1alpha1.Vnic{}).
