...
// +kubebuilder:rbac:groups=tag.vsphere.kubeform.com,resources=categories,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=tag.vsphere.kubeform.com,resources=categories/status,verbs=get;update;patch

func (r *CategoryReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("category", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Category")
...
func (r *CategoryReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &tagv1alpha1.Category{}); err != nil {
			klog.Error(err, "unable to set up auditor", tagv1alpha1.Category{}.APIVersion, tagv1alpha1.Category{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&tagv1alpha1.Category{}).
