...
// VmOverrideReconciler reconciles a VmOverride object
type VmOverrideReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=drs.vsphere.kubeform.com,resources=vmoverrides,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=drs.vsphere.kubeform.com,resources=vmoverrides/status,verbs=get;update;patch

func (r *VmOverrideReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("vmoverride", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch VmOverride")
...
func (r *VmOverrideReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &drsv1alpha1.VmOverride{}); err != nil {
			klog.Error(err, "unable to set up auditor", drsv1alpha1.VmOverride{}.APIVersion, drsv1alpha1.VmOverride{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&drsv1alpha1.VmOverride{}).
