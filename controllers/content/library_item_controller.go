...
// LibraryItemReconciler reconciles a LibraryItem object
type LibraryItemReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=content.vsphere.kubeform.com,resources=libraryitems,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=content.vsphere.kubeform.com,resources=libraryitems/status,verbs=get;update;patch

func (r *LibraryItemReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("libraryitem", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch LibraryItem")
...
func (r *LibraryItemReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &contentv1alpha1.LibraryItem{}); err != nil {
			klog.Error(err, "unable to set up auditor", contentv1alpha1.LibraryItem{}.APIVersion, contentv1alpha1.LibraryItem{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&contentv1alpha1.LibraryItem{}).
