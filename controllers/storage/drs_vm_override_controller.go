...
// DrsVmOverrideReconciler reconciles a DrsVmOverride object
type DrsVmOverrideReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=storage.vsphere.kubeform.com,resources=drsvmoverrides,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=storage.vsphere.kubeform.com,resources=drsvmoverrides/status,verbs=get;update;patch

func (r *DrsVmOverrideReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("drsvmoverride", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DrsVmOverride")
...
func (r *DrsVmOverrideReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &storagev1alpha1.DrsVmOverride{}); err != nil {
			klog.Error(err, "unable to set up auditor", storagev1alpha1.DrsVmOverride{}.APIVersion, storagev1alpha1.DrsVmOverride{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&storagev1alpha1.DrsVmOverride{}).
